...
public class FromMapFix extends BaseCreateMethodsFix<DartComponent> {
    ConfigurationData mData;

    public FromMapFix(@NotNull DartClass dartClass) {
        super(dartClass);
    }
    ...
    protected Template buildFunctionsText(TemplateManager templateManager, Set<DartComponent> elementsToProcess) {
        ...
        template.addVariable(new TextExpression(mData.fromMapMethodName), true);
        template.addTextSegment("(Map<String, dynamic> map)");
        template.addTextSegment("{");
